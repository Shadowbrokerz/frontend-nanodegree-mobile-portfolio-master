...
var resizePizzas = function(size) {
 ...
 function applyClass(size){
  var pizza = document.getElementsByClassName("randomPizzaContainer");
  for(i=0;i<pizza.length;i++){
        pizza[i].className = ['randomPizzaContainer '+size];
    }
  ...
  // Applying a CSS class for less resource consumption.
  function changeSliderLabel(size) {
    var pizzaSlider =  document.querySelector("#pizzaSize");
    switch(size) {
      case "1":
  ...
  function determineDx (elem, size) {
    ...
    //Replaced document.querySelector with .getElementById
...
//Moved this outside the loop to avoid declaration in excess.
...
function updatePositions(e) {
  if(e.timeStamp - timeStamp > 60){
    frame++;
    window.performance.mark("mark_start_frame");
    //Replaced with getELementsByClassname.
    var items = document.getElementsByClassName('mover');
    //Declared a variable to avoid calling it everytime.
    var top = document.body.scrollTop / 1250;
    for (var i = 0, len = items.length; i < len; i++) {
      var phase = Math.sin(top + (i % 5));
      //Replaced translate for translateX because it was breaking the pizza movement.
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  updatePositions({timeStamp: Date.now()});
});
